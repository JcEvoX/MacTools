...
> **macOS 27 说明：** 新的单窗口菜单栏宿主不再通过常规通道把右键事件传递给第三方菜单栏图标。MacTools 通过只读的辅助功能事件监听恢复了右键——每次点击实时读取图标位置进行命中，因此需**授予「辅助功能」权限**后才生效。也可用 **Option+左键** 点击 MacTools 图标打开次级面板，无需权限。
